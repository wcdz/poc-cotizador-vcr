...
class TablaMortalidadRepository(ABC):
    ...
    @abstractmethod
    def get_tasa_mortalidad(self, edad: int, sexo: Sexo, fumador: EstadoFumador) -> float:
...
class JsonTablaMortalidadRepository(TablaMortalidadRepository):
    ...
    def get_tasa_mortalidad(self, edad: int, sexo: Sexo, fumador: EstadoFumador) -> float:
        ...
        if clave not in tabla_mortalidad[edad_str]:
            raise ValueError(f"No se encontró la combinación de sexo y estado de fumador para la edad {edad}")
