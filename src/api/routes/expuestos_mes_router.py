...
class ProyeccionActuarialInput(BaseModel):
    edad_actuarial: int = Field(
        ..., ge=18, le=70, description="Edad actuarial del asegurado"
    )
    ...
    periodo_vigencia: int = Field(
        ..., ge=1, le=50, description="Período de vigencia en años"
    )
    periodo_pago_primas: int = Field(
        ..., ge=1, le=50, description="Período de pago de primas en años"
    )
    ...
    ajuste_mortalidad: float = Field(
        1.0, gt=0, description="Factor de ajuste para la tabla de mortalidad"
    )
